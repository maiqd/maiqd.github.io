...
# AI tool integration (UI only)

Use these files **only** when designing, styling, or refactoring visual UI — not for config, content, analytics, or tooling.

| Tool | File | How it scopes to UI |
|------|------|---------------------|
| **Cursor** | `.cursorrules` + `.cursor/rules/design-ui.mdc` | Root file states UI-only activation; `.mdc` auto-attaches via globs on `_sass/`, `_includes/`, `_layouts/`, `assets/css/`, `index.html`, `design/`. Use @Codebase on those paths. |
| **GitHub Copilot** | `.github/instructions/design-ui.instructions.md` | `applyTo` frontmatter limits rules to UI paths. `.github/copilot-instructions.md` points here (repo-wide file stays minimal). |
| **Claude** | Claude Projects → Project Knowledge | Upload `design/design-system.md` and `design/claude-project-knowledge.md`. `.clauderules` mirrors scope for file-based tools. |
| **Open Code / Windsurf** | `.clauderules`, `.windsurfrules` | Explicit “UI design only” activation + path list; full spec in `design/design-system.md`. |
